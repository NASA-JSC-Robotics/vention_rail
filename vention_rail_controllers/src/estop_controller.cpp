...
EstopController::command_interface_configuration() const {
...
controller_interface::InterfaceConfiguration
vention_rail_controllers::EstopController::state_interface_configuration()
    const {
  controller_interface::InterfaceConfiguration config;
  config.type = controller_interface::interface_configuration_type::INDIVIDUAL;
...
vention_rail_controllers::EstopController::update(
...
vention_rail_controllers::EstopController::on_configure(
...
controller_interface::CallbackReturn
vention_rail_controllers::EstopController::on_activate(
    const rclcpp_lifecycle::State & /*previous_state*/) {
  try {
    auto program_state_pub_qos = rclcpp::SystemDefaultsQoS();
    program_state_pub_qos.transient_local();
    program_state_pub_ = get_node()->create_publisher<std_msgs::msg::Bool>(
        "~/robot_program_running", program_state_pub_qos);
  } catch (...) {
...
controller_interface::CallbackReturn
vention_rail_controllers::EstopController::on_deactivate(
    const rclcpp_lifecycle::State & /*previous_state*/) {
  try {
    // reset publisher
...
PLUGINLIB_EXPORT_CLASS(vention_rail_controllers::EstopController,
