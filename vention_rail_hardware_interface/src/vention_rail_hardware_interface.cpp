...
CallbackReturn
RailEHardwareInterface::on_init(const hardware_interface::HardwareInfo &info_) {
  ...
  acceleration_limit =
      stof(system_info.hardware_parameters["acceleration_limit"]);
  safety_com_port = system_info.hardware_parameters["safety_com_port"];
  return CallbackReturn::SUCCESS;
}

CallbackReturn RailEHardwareInterface::on_configure(
...
CallbackReturn RailEHardwareInterface::on_cleanup(
    const rclcpp_lifecycle::State & /*previous_state*/) {
  run_ = false;
  com_thread_.join();
...
RailEHardwareInterface::export_state_interfaces() {
...
RailEHardwareInterface::export_command_interfaces() {
...
CallbackReturn RailEHardwareInterface::on_activate(
    const rclcpp_lifecycle::State & /*previous_state*/) {
  ...
  while (!is_safe) {
    ...
    is_safe = (is_safe_string.find(pressed) !=
               std::string::npos); // if 113 is in the read serial message
  ...
  try {
    ...
    if (response.find("error") != string::npos) {
      ...
      while (true) {
        ...
        RCLCPP_INFO(rclcpp::get_logger("RailEHardwareInterface"),
                    "Motion complete: %s", response.c_str());
...
CallbackReturn RailEHardwareInterface::on_deactivate(
    const rclcpp_lifecycle::State & /*previous_state*/) {
...
RailEHardwareInterface::read(const rclcpp::Time & /*time*/,
  ...
  RCLCPP_DEBUG(rclcpp::get_logger("RailEHardwareInterface"),
...
hardware_interface::return_type
RailEHardwareInterface::write(const rclcpp::Time & /*time*/,
                              const rclcpp::Duration & /*period*/) {
...
void RailEHardwareInterface::com_thread() {
  ...
  while (run_) {
    ...
    int64_t dt_com = std::chrono::duration_cast<std::chrono::nanoseconds>(
                         curr_time - last_time)
                         .count();
    ...
    std::string pos_str =
        sendHTTPMessage(create_get_position_command(), sockfd_);
    // if the returned string contains "error", don't try to parse
    std::string estop_status_str =
        sendHTTPMessage(create_estop_status_command(), sockfd_);
    ...
    if (hard_estopped) {
      ...
      if (!isnan(parsed_position)) {
        ...
        curr_velocity_ = (curr_position_ - previous_position) /
                         (static_cast<double>(dt_com) / 1.0e9);
      } else {
        // estimate our current position based on our previous velocity if we
        // received invalid data
        curr_position_ = curr_position_ +
                         curr_velocity_ * (static_cast<double>(dt_com) / 1.0e9);
      }
    }
    ...
    if (soft_estopped) {
      soft_estopped = (abs(curr_position_ - hw_commands_positions_[0]) >
                       soft_estop_tolerance) ||
                      hard_estopped;
    }
    ...
    double unclamped_vel_cmd_pid =
        pid_.computeCommand(position_cmd_ - hw_states_positions_[0], dt_com);
    double velocity_cmd =
        std::clamp(unclamped_vel_cmd_pid, -velocity_limit, velocity_limit);
...
PLUGINLIB_EXPORT_CLASS(vention_rail_hardware_interface::RailEHardwareInterface,
