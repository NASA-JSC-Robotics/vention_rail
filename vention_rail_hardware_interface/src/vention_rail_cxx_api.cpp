...
int connect_to_rail(string host, const int portno, const float timeout)
{
  ...
  if (sockfd < 0)
  {
    RCLCPP_FATAL(rclcpp::get_logger("RailEHardwareInterface"), "Error opening socket to Rail");
  }
  ...
  if (server == NULL)
  {
    RCLCPP_FATAL(rclcpp::get_logger("RailEHardwareInterface"), "Error finding IP Address to Rail");
  }
  ...
  {
    RCLCPP_FATAL(rclcpp::get_logger("RailEHardwareInterface"), "Error connecting to Rail");
  }
  ...
  if (setsockopt(sockfd, SOL_SOCKET, SO_RCVTIMEO, &timeout_, sizeof timeout_) < 0)
  {
    RCLCPP_FATAL(rclcpp::get_logger("RailEHardwareInterface"), "Error setting socket timeout");
  }
...
string sendHTTPMessage(string message_fmt, int sockfd)
{
  ...
  do
  {
    ...
    if (bytes < 0)
    {
      RCLCPP_FATAL(rclcpp::get_logger("RailEHardwareInterface"), "Error writing message to Rail");
    }
    if (bytes == 0)
      break;
    sent += bytes;
  ...
  if (bytes < 0)
  {
    RCLCPP_WARN(rclcpp::get_logger("RailEHardwareInterface"), "ERROR reading response from socket");
  }
  else
  {
    received += bytes;
