...
void reset_estop() { stopped = false; }

string create_estop_status_command() { return "estop/status;"; }

string create_homing_command() { return "im_home_axis_1;"; }

string create_motion_complete_command() { return "isMotionCompleted;"; }
...
string create_set_position_command(double position) {
  // This is a bug in the MachineMotion firmware and should only require a value
  // of 1000
...
string create_set_max_vel_command(double max_velocity) {
  int max_vel_mm_s =
      static_cast<int>(max_velocity * 100000.0); // convert from m/s to mm/s
  return "SET speed/" + to_string(max_vel_mm_s) + "/;";
}

string create_set_max_acc_command(double max_acceleration) {
  int max_acc_mm_s2 =
      static_cast<int>(max_acceleration * 2000.0); // convert from m/s to mm/s
...
string create_velocity_command(double velocity, double accel) {
  ...
  int acc_cmd =
      static_cast<int>(accel * 2000.0); // convert from m/s^2 to mm/s^2
  string command_str =
      "SET im_conv_1 S" + to_string(vel_cmd) + " A" + to_string(acc_cmd) + ";";
  return command_str;
}
...
double parse_position_string(std::string pos_str) {
  ...
  if (open_index != std::string::npos) {
    ...
    RCLCPP_ERROR(rclcpp::get_logger("RailEHardwareInterface"),
                 "Returned position string: '%s' was not of format "
                 "(<position>). Not updating position.",
                 pos_str_input.c_str());
  ...
  try {
    float position =
        static_cast<double>(stoi(pos_str)) / 1000.0; // convert mm to m
    return position;
  } catch (const std::exception &e) {
    RCLCPP_ERROR(
        rclcpp::get_logger("RailEHardwareInterface"),
        "Was not able to parse '%s' into a float. Not updating position.",
        pos_str.c_str());
    return std::numeric_limits<double>::quiet_NaN();
  }
}
...
int connect_to_rail(string host, const int portno, const float timeout) {
  ...
  timeout_.tv_usec = static_cast<int>(fraction) *
                     1000000; // convert fraction into microseconds

  if (setsockopt(sockfd, SOL_SOCKET, SO_RCVTIMEO, &timeout_, sizeof timeout_) <
      0) {
    RCLCPP_FATAL(rclcpp::get_logger("RailEHardwareInterface"),
